...
const DetailsRestaurantScreen = ({navigation, route}) => {
  ...
  return (
    <View style={styles({schema}).rootContainer}>
      ...
      <View>
        <Image
          source={Images.Restaurant_Photo}
...
const PopularMenuList = ({navigation}) => {
    ...
    <View>
      ...
      {scrollHorizontal ? (
        <FlatList
          ...
          data={menuData}
          renderItem={({item}) => (
            <MenuBox data={item} navigation={navigation} />
          )}
  ...
  return (
    <View style={{gap: heightPixel(20)}}>
      ...
      <FlatList
        scrollEnabled={false}
        data={Ratting}
        renderItem={({item}) => <RattingBox data={item} />}
        keyExtractor={item => item.id}
        style={styles(schema).flateList}
      />
    </View>
...
export default DetailsRestaurantScreen;
